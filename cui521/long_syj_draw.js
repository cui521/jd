/*
 ...
 [task_local]
 #赚京豆助力
 15,35,55 0-1 * * * https://raw.githubusercontent.com/3028/update/master/tuan/long_syj_draw.js, tag=赚京豆助力, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_syj.png, enabled=true

 ================Loon==============
 [Script]
 cron "15,35,55 0-1 * * *" script-path=https://raw.githubusercontent.com/3028/update/master/tuan/long_syj_draw.js, tag=赚京豆助力

 ===============Surge=================
 赚京豆助力 = type=cron,cronexp="15,35,55 0-1 * * *",wake-system=1,timeout=3600,script-path=hhttps://raw.githubusercontent.com/3028/update/master/tuan/long_syj_draw.js

 ============小火箭=========
 赚京豆助力 = type=cron,script-path=https://raw.githubusercontent.com/3028/update/master/tuan/long_syj_draw.js, cronexpr="15,35,55 0-1 * * *", timeout=3600, enable=true
 */
const $ = new Env('赚京豆瓜分');
...
!(async () => {
  ...
  await getAuthorShareCode(`http://service-kazgm833-1255594201.sh.apigw.tencentcs.com:80/jd/tuan?appKey=long_syj&action=query`);
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (cookiesArr[i]) {
      ...
      if ($.canHelp) {
        ...
        if ($.authorTuanList.length) console.log(`开始账号互助 赚京豆-瓜分京豆 活动`)
        for (let j = 0; j < $.authorTuanList.length; ++j) {
          console.log(`账号 ${$.UserName} 开始给 ${$.authorTuanList[j]['assistedPinEncrypted']}助力`)
    ...
    $.get(options, async (err, resp, data) => {
      try {
        if (err) {
          ...
          console.log(`远程团ID列表: `, $.authorTuanList)
        }
      } catch (e) {
        $.logErr(e, resp)
      } finally {
